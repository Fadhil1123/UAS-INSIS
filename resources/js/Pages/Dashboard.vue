  ...
  <AuthenticatedLayout>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
      ...
      <div class="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-xs flex items-center justify-between group hover:border-indigo-200 transition-all duration-300">
        ...
        <div class="w-12 h-12 bg-indigo-50 rounded-xl text-indigo-600 flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
      ...
      <div class="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-xs flex items-center justify-between group hover:border-indigo-200 transition-all duration-300">
        <div>
          <p class="text-xs font-semibold text-slate-400 uppercase tracking-wider">Peminjaman Menunggu Persetujuan</p>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
      <!-- Upcoming Bookings Panel (2 Columns span) -->
      <div class="lg:col-span-2 bg-white rounded-3xl border border-slate-200/80 shadow-xs p-6">
        <div class="flex items-center justify-between mb-6">
          <h2 class="text-lg font-bold text-slate-900 flex items-center gap-2">
            ...
            Jadwal Booking Terdekat Anda
          </h2>
          <Link href="/bookings" class="text-xs font-bold text-indigo-600 hover:text-indigo-700 transition-colors">
            Lihat Semua Riwayat →
          </Link>
        </div>
        ...
        <div v-if="upcomingBookings && upcomingBookings.length > 0" class="space-y-4">
          ...
          >
            <div class="flex items-center gap-4">
              <div class="w-12 h-12 rounded-xl bg-indigo-50 flex flex-col items-center justify-center text-indigo-600">
        ...
        <div v-else class="text-center py-12 bg-slate-50/50 rounded-2xl border border-dashed border-slate-200">
          ...
          <p class="text-xs text-slate-400 mt-1">Silakan ajukan peminjaman ruangan terlebih dahulu.</p>
          <Link href="/rooms" class="mt-4 inline-flex items-center gap-2 px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-xl text-xs shadow-md shadow-indigo-600/10 transition-colors">
            Cari Ruangan
          </Link>
        </div>
      </div>

      <!-- Quick Tips / Information Sidebar (1 Column span) -->
      <div class="bg-white rounded-3xl border border-slate-200/80 shadow-xs p-6 flex flex-col justify-between">
        <div>
          ...
          <ul class="space-y-4">
            <li class="flex gap-3">
              <span class="flex-shrink-0 w-6 h-6 rounded-full bg-indigo-50 text-indigo-600 flex items-center justify-center font-bold text-xs">1</span>
              <div>
                <p class="text-xs font-bold text-slate-800">Cari Ruangan</p>
                <p class="text-[11px] text-slate-400 mt-0.5 leading-relaxed">Pilih menu "Daftar Ruangan" lalu tentukan ruangan yang ingin digunakan.</p>
              </div>
            </li>
            <li class="flex gap-3">
              <span class="flex-shrink-0 w-6 h-6 rounded-full bg-indigo-50 text-indigo-600 flex items-center justify-center font-bold text-xs">2</span>
              <div>
                <p class="text-xs font-bold text-slate-800">Cek Kalender Ketersediaan</p>
                <p class="text-[11px] text-slate-400 mt-0.5 leading-relaxed">Cek slot waktu kosong yang tidak bertabrakan dengan jadwal kuliah (SIMARI) atau booking lain.</p>
              </div>
            </li>
            <li class="flex gap-3">
              <span class="flex-shrink-0 w-6 h-6 rounded-full bg-indigo-50 text-indigo-600 flex items-center justify-center font-bold text-xs">3</span>
              <div>
                <p class="text-xs font-bold text-slate-800">Kirim Pengajuan</p>
                <p class="text-[11px] text-slate-400 mt-0.5 leading-relaxed">Isi form pengajuan peminjaman dengan lengkap beserta keperluan acara.</p>
              </div>
            </li>
          </ul>
        </div>
        ...
        <div class="mt-8 pt-6 border-t border-slate-100 text-[11px] text-slate-400 leading-relaxed font-medium">
          <strong>Catatan:</strong> Status peminjaman akan otomatis dikirimkan ke nomor WhatsApp Anda yang terdaftar ketika admin memproses pengajuan.
        </div>
      </div>
    </div>
